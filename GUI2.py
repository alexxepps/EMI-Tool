...
class NoiseImpedanceWindow(Window):
    ...
    def create_input_frame(self):
        ...
        button(bt6_frame, "Calculate Noise Impedance", self.calculate_noise_impedance)
    ...
    def calculate_noise_impedance(self):
        global filter_choke, CM_base, CM_filtered, Noise_Floor, CM_Zs, DM_base, DM_filtered, DM_Zs, impedance_xcap, nonideal_toggle_x, impedance_xcap2, nonideal_toggle_x_2
...
class FilterCM(Window):
    ...
    def setup_interface(self):
        ...
        # noise limit
        self.margin = label_entry(options_frame, "Noise Limit Margin [dBuV]", 13)
    ...
    def show_suggested_impedance(self):
        global CM_est, y_cap, CM_Topo, y_cap_1, nonideal_toggle_y, impedance_ycap, impedance_ycap_1, topology_checks_out
    ...
    def show_actual_impedance(self):
        ...
        if CM_est == None:
            ...
            if CM_Topo is None:
                ...
                #actual_impedance.plot(0, CM_base.freq, CM_est.worst_attenuation, "TP1: Attenuation Performance")
            elif CM_Topo == 3:
        ...
        print(mean_noise)
        #limit_low_frequency = np.full_like(5901, 66)
        #limit_high_frequency = np.full_like(5901, 60)
        limit_low_frequency = 66
        limit_high_frequency = 60
        #new_array = np.where(71, limit_high_frequency, limit_low_frequency)
...
class FilterDM(Window):
    ...
    def setup_interface(self):
        ...
        label = ttk.Label(options_frame, text="Press Run button to determine which topology provides the greatest attenuation")
        label.pack()
        button(options_frame, "Run", combine_funcs(self.DM_topology_compare_attenuation, self.show_actual_impedance))
        select_button.pack(side="left")
    ...
    def show_actual_impedance(self):
        global leakage_choke, DM_est, DM_Topo, y_cap
        ...
        #print("Input test xcap")
        #print(x_cap_0)
        #print("Input test ycap")
        #print(y_cap)
        #print("Leakage choke")
        #print(leakage_choke)
        ...
        if DM_Topo < 6:
            if nonideal_toggle_x == 0:
                ...
                if DM_Topo == 1:
                    actual_impedance.plot(0, impedance_xcap_actual_1.freq, impedance_xcap_actual_1.impedance, "Actual X2 Capacitor Impedance")
                actual_impedance.log_scale(0)
                actual_impedance.prettify(0, 'Impedance[$\Omega$]', 'Impedance Vs Freq, Log Scale')
        elif DM_Topo == 6:
...
class automate(Window):
    ...
    def automate(self):
        ...
        """CM_est.LC_topology_array_input(impedance_ycap, filter_choke)
        CM_est.calculate_noise(CM_base)
        self.check_average_noise_max(CM_est.worst_estimate)
        if topology_checks_out == 2:
            label = ttk.Label(options_frame, text="CM Topology 1 is the best for this noise source")
            label.pack()
        ...
        self.check_average_noise_max(CM_est.worst_estimate)
        if topology_checks_out == 2:
            label = ttk.Label(options_frame, text="CM Topology 2 is the best for this noise source")
            label.pack()
        
        CM_est.CLC_topology_array_input(impedance_ycap, impedance_ycap, filter_choke)
        CM_est.calculate_noise(CM_base)
        self.check_average_noise_max(CM_est.worst_estimate)
        if topology_checks_out == 2:
            label = ttk.Label(options_frame, text="CM Topology 3 is the best for this noise source")
            label.pack()"""
    ...
    """def pull_multiple_ycap_files(self, filename):
        path = filename
        #path = "C://Users//240036042//Documents//Laundry 2024//mymeas//capacitor impedance curves//xcaps"
        files = os.listdir(path)
        for file in files:
            ycap_imped = Capacitor_model.Cap(file)
            ycap_imped_1 = Capacitor_model.Cap(file)
            return ycap_imped, ycap_imped_1"""
