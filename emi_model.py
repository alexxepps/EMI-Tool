...
from Capacitor_model import Cap
...
class Spectrum_Measurement:
    ...
    def get_de_floored(self, floor, pts = 3, delta = 3, window = 6):
        ...
        #filtered_measurement = self.measurement
...
class Noise_Impedance:
    ...
    def __init__(self, baseline: Spectrum_Measurement, filtered: Spectrum_Measurement, floor: Spectrum_Measurement, min_freq: float = 150000, max_freq: float = 30000000) -> None:
        ...
        baseline_no_floor = baseline
        filtered_no_floor = filtered
        ...
        #self.attenuation = 10 ** ((self.baseline_measurement - self.filtered_measurement)/20)
        self.attenuation = np.abs(10 ** ((self.baseline_measurement - self.filtered_measurement)/20)) 

        #att_test = 10 ** ((self.baseline_measurement - self.filtered_measurement)/20)
        #for att_test_pt in att_test:
            #if att_test_pt < 0:
                #att_test_pt 
    ...
    def CM_calculation_include_ycaps(self, choke:Choke, y_cap_impedance1:Cap, y_cap_impedance2:Cap, freq:np.ndarray):
        Z_choke = np.delete(choke.impedance, np.where(self.remove_mask))
        Y_cap_1 = np.delete(y_cap_impedance1.impedance, np.where(self.remove_mask))
        Y_cap_2 = np.delete(y_cap_impedance2.impedance, np.where(self.remove_mask))

        twentyfive = np.full_like(Z_choke, 25)
        one = np.ones_like(self.freq)

        N1 = self.attenuation * twentyfive**2 * Y_cap_1 * Y_cap_2 + self.attenuation * twentyfive * Z_choke * Y_cap_1 * Y_cap_2 - twentyfive**2 * Y_cap_1 * Y_cap_2 
        N2 = -twentyfive * Z_choke * Y_cap_1 * Y_cap_2 - Z_choke * twentyfive * Y_cap_1 * Y_cap_2 - Z_choke**2 * Y_cap_1 * Y_cap_2

        D1 = -self.attenuation * twentyfive * Y_cap_1 * Y_cap_2 - self.attenuation * Z_choke * Y_cap_1 * Y_cap_2 + twentyfive * Y_cap_1 * Y_cap_2 + twentyfive**2 * Y_cap_1
        D2 = twentyfive * Y_cap_1 * Z_choke + Y_cap_2 * twentyfive**2 + Y_cap_2 * Z_choke * twentyfive + Z_choke * Y_cap_1 * Y_cap_2 
        D3 = Z_choke * Y_cap_1 * twentyfive + Z_choke**2 * Y_cap_1 + Y_cap_2 * twentyfive * Z_choke + Y_cap_2 * Z_choke**2

        self.min = np.abs((N1 * N2)/(D1 * D2 * D3))
        self.max = np.abs((N1 * N2)/(D1 * D2 * D3))

        self.fill_min = self.min
        self.fill_max = self.max
    ...
    def DM_calculation(self, capacitance, freq:np.ndarray):
        ...
        #x_cap = np.delete(x_cap_impedance.impedance, np.where(self.remove_mask))
        ...
        #self.min = np.abs(one / (alpha_minus))
        #self.max = np.abs(one / (alpha_plus))
    ...
    # calculate min, max, geo mean noise impedance for Differential Mode, requires X capacitor used. See documentation for background on calculations------------------------------------------------------------------
    def DM_calculation_actual(self, x_cap_impedance:Cap, freq:np.ndarray):
        # usually fine to use ideal cap model
        x_cap = np.delete(x_cap_impedance.impedance, np.where(self.remove_mask))
    ...
    # calculate min, max, geo mean noise impedance for Differential Mode, requires X capacitor used. See documentation for background on calculations------------------------------------------------------------------
    def DM_calculation_actual_2xcaps(self, x_cap_impedance:Cap, x_cap_impedance2:Cap, freq:np.ndarray):
        # usually fine to use ideal cap model
        x_cap = np.delete(x_cap_impedance.impedance, np.where(self.remove_mask))
        x_cap_2 = np.delete(x_cap_impedance2.impedance, np.where(self.remove_mask))
        x_cap_total = self.parallel_eqv(x_cap, x_cap_2)

        onehundred = np.full_like(self.freq, 100)
        one = np.ones_like(self.freq)

        #back-calculate noise source impedance
        alpha_plus = np.abs(1 / (np.abs(x_cap_total*self.attenuation) + np.abs(x_cap_total)))
        alpha_minus = np.abs(1 / (np.abs(x_cap_total*self.attenuation) - np.abs(x_cap_total)))
        #self.min = np.abs(one / (alpha_minus))
        #self.max = np.abs(one / (alpha_plus))
...
class Common_Mode_Estimate(Estimate):
    ...
    def CL_topology_helper(self, Z_noise_source: np.ndarray) -> np.ndarray:
        ...
        for attenuation_point in attenuation:
            if attenuation_point <= 0:
                attenuation_point = 0
    ...
    def CL_topology(self, y_cap: float, cm_choke: Choke):
        ...
        self.worst_attenuation = np.minimum(attenuation_w_min_noise, attenuation_w_max_noise)
        return
    
    def CL_topology_array_input(self, y_cap: Cap, cm_choke: Choke):
        # Recall that in the CM equivalent circuit, both y caps are in parallel...
        #self.Z_cap = -1j / (2*math.pi*self.freq*y_cap*2)
        self.Z_choke = cm_choke.impedance
        self.Z_cap = y_cap.impedance
    ...
    def LC_topology_array_input(self, y_cap: Cap, cm_choke: Choke):
        # Recall that in the CM equivalent circuit, both y caps are in parallel...
        #self.Z_cap = -1j / (2*math.pi*self.freq*y_cap*2)
        self.Z_choke = cm_choke.impedance
        self.Z_cap  = y_cap.impedance
    ...
    def CLC_topology_helper(self, Z_noise_source: np.ndarray) -> np.ndarray:
        ...
        filter = (Z3 / (Z3 + Z_noise_source)) * (Z1 / (Z1 + self.Z_choke))
        attenuation = no_filter / filter
    ...
    def find_Z_choke_helper_LC(self, Z_noise_source: np.ndarray) -> np.ndarray:
        ...
        for noise_pt, attenuation_pt, z_cap_pt in zip(Z_noise_source, self.needed_attenuation, self.Z_cap):
            ...
            B = -attenuation_pt * z_cap_pt * self.R_lisn**2 - attenuation_pt * z_cap_pt * self.R_lisn * noise_pt + self.R_lisn**2 * z_cap_pt + noise_pt * self.R_lisn**2 + self.R_lisn**2 * z_cap_pt + noise_pt * self.R_lisn**2 + noise_pt * z_cap_pt * self.R_lisn

            C = -attenuation_pt * self.R_lisn**3 * z_cap_pt - attenuation_pt * self.R_lisn**2 * z_cap_pt * noise_pt + self.R_lisn**3 * z_cap_pt + noise_pt * self.R_lisn**3 + noise_pt * z_cap_pt * self.R_lisn**2
...
class Differential_Mode_Estimate(Estimate):
    # setup
    def __init__(self) -> None:
        super().__init__(100) # sets resistance value for Rload
    ...
    def PI_topology_helper(self, Z_noise_source:np.ndarray):
        no_filter = self.R_lisn / (self.R_lisn + Z_noise_source)
        Z1 = self.parallel_eqv(self.R_lisn, self.Z_y_cap)
        Z2 = Z1 + self.Z_leakage
        Z3 = self.parallel_eqv(Z2, self.Z_x_cap)
        Vn1 = Z3 / (Z3 + Z_noise_source)
        filter = Vn1 * (Z1 / (Z1 + self.Z_leakage))
        A = no_filter / filter
        return np.abs(A)
    ...
    def PI_topology(self, x_cap: float, y_cap:float, choke_leakage:Choke):
        ...
        self.worst_attenuation = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        self.worst_attenuation_PI = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        return
    
    def PI_topology_array_input(self, x_cap: Cap, y_cap: Cap, choke_leakage:Choke):
        self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def PI_topology_math(self, y_cap:float, choke_leakage:Choke):
        #self.Z_x_cap = -1j / (2*math.pi*self.freq*x_cap)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = -1j / (2*math.pi*self.freq*y_cap/2)
        self.Z_leakage = choke_leakage.impedance
    ...
    def PI_topology_math_array_input(self, y_cap:Cap, choke_leakage:Choke):
        #self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def PI2_topology_helper(self, Z_noise_source:np.ndarray):
        no_filter = self.R_lisn / (self.R_lisn + Z_noise_source)
        Z1 = self.parallel_eqv(self.R_lisn, self.Z_x_cap_1)
        Z2 = self.parallel_eqv(self.Z_y_cap, self.Z_x_cap_2) 
        Z3 = Z1 + self.Z_leakage 
        Z4 = self.parallel_eqv(Z3, Z2)
        filter = (Z4 / (Z4 + Z_noise_source))*(Z1 / (Z1 + self.Z_leakage))
        A = no_filter / filter
    ...
    def PI2_topology(self, x_cap_1: float, x_cap_2: float, y_cap:float, choke_leakage:Choke):
        ...
        self.worst_attenuation = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        self.worst_attenuation_PI2 = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        return
    
    def PI2_topology_array_input(self, x_cap_1: Cap, x_cap_2: Cap, y_cap: Cap, choke_leakage:Choke):
        
        self.Z_x_cap_1 = x_cap_1.impedance
        #print("x_cap_1")
        #print(x_cap_1)
        self.Z_x_cap_2 = x_cap_2.impedance
        #print("x_cap_2")
        #print(x_cap_2)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
    ...
    def PI2_topology_math(self, y_cap:float, choke_leakage:Choke):
        #self.Z_x_cap = -1j / (2*math.pi*self.freq*x_cap)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = -1j / (2*math.pi*self.freq*y_cap/2)
        self.Z_leakage = choke_leakage.impedance
    ...
    def PI2_topology_math_array_input(self, y_cap: Cap, choke_leakage:Choke):
        #self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def CCL_topology(self, x_cap: float, y_cap:float, choke_leakage:Choke):
        ...
        self.worst_attenuation = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        self.worst_attenuation_CyCxL = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        return
    
    def CCL_topology_array_input(self, x_cap: Cap, y_cap: Cap, choke_leakage:Choke):
        self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def CCL_topology_math(self, y_cap:float, choke_leakage:Choke):
        #self.Z_x_cap = -1j / (2*math.pi*self.freq*x_cap)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = -1j / (2*math.pi*self.freq*y_cap/2)
        self.Z_leakage = choke_leakage.impedance
    ...
    def CCL_topology_math_array_input(self, y_cap: Cap, choke_leakage:Choke):
        #self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def LCC_topology_helper(self, Z_noise_source:np.ndarray):
        ...
        filter = (Z3 / (Z3 + Z_noise_source)) * ((self.R_lisn) / (self.R_lisn + self.Z_leakage))
        A = no_filter / filter
        return np.abs(A)
    ...
    def LCC_topology(self, x_cap: float, y_cap:float, choke_leakage:Choke):
        ...
        self.worst_attenuation = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        self.worst_attenuation_LCyCx = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        return
    
    def LCC_topology_array_input(self, x_cap: Cap, y_cap: Cap, choke_leakage:Choke):
        self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def LCC_topology_math(self, y_cap:float, choke_leakage:Choke):
        #self.Z_x_cap = -1j / (2*math.pi*self.freq*x_cap)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = -1j / (2*math.pi*self.freq*y_cap/2)
        self.Z_leakage = choke_leakage.impedance
    ...
    def LCC_topology_math_array_input(self, y_cap: Cap, choke_leakage:Choke):
        #self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def CxLCy_topology(self, x_cap: float, y_cap:float, choke_leakage:Choke):
        ...
        self.worst_attenuation = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        self.worst_attenuation_CxLCy = np.minimum(np.abs(A_min_noise),np.abs(A_max_noise))
        return
    
    def CxLCy_topology_array_input(self, x_cap: Cap, y_cap: Cap, choke_leakage:Choke):
        self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def CxLCy_topology_math(self, y_cap:float, choke_leakage:Choke):
        #self.Z_x_cap = -1j / (2*math.pi*self.freq*x_cap)
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = -1j / (2*math.pi*self.freq*y_cap/2)
        self.Z_leakage = choke_leakage.impedance
    ...
    def CxLCy_topology_math_array_input(self, y_cap: Cap, choke_leakage:Choke):
        #self.Z_x_cap = x_cap.impedance
        # y caps will be in series for the DM equiv circuit...
        self.Z_y_cap = y_cap.impedance + y_cap.impedance
        self.Z_leakage = choke_leakage.impedance
    ...
    def find_Zx_CxLCy(self, base: Spectrum_Measurement, limit: np.ndarray):
        ...
        self.needed_Z_x_CxLCy = np.maximum(self.find_Z_x_helper_CxLCy(self.min_noise),self.find_Z_x_helper_CxLCy(self.max_noise))
        return
    ...
    def find_Z_x_helper_CxLCy(self, Z_noise_source:np.ndarray):
        ...
        for noise_pt, attenuation_pt, z_cap_y_pt, z_leak_pt in zip(Z_noise_source, self.needed_attenuation, self.Z_y_cap, self.Z_leakage):
            ...
            A = (z_leak_pt * z_cap_y_pt * self.R_lisn**2) + (noise_pt * z_leak_pt * self.R_lisn**2) + (noise_pt * z_cap_y_pt * self.R_lisn**2) + (z_leak_pt**2 * z_cap_y_pt * self.R_lisn) + (noise_pt * z_leak_pt**2 * self.R_lisn) + (noise_pt * z_cap_y_pt * z_leak_pt * self.R_lisn) - (attenuation_pt * z_leak_pt * z_cap_y_pt * self.R_lisn**2) - (attenuation_pt * z_leak_pt * z_cap_y_pt * self.R_lisn * noise_pt)

            B = (z_leak_pt * self.R_lisn**3 * z_cap_y_pt) + (noise_pt * z_leak_pt *self.R_lisn**3) + (noise_pt * z_cap_y_pt * self.R_lisn**3) + (z_leak_pt**2 * z_cap_y_pt * self.R_lisn**2) + (noise_pt * z_leak_pt**2 * self.R_lisn**2) + (noise_pt * z_cap_y_pt * z_leak_pt * self.R_lisn**2) + (z_leak_pt**2 * self.R_lisn**2 * z_cap_y_pt) + (noise_pt * z_leak_pt**2 * self.R_lisn**2) + (noise_pt * z_cap_y_pt * self.R_lisn**2 * z_leak_pt) - (attenuation_pt * z_leak_pt * self.R_lisn**3 * z_cap_y_pt) - (attenuation_pt * z_leak_pt * self.R_lisn**2 * z_cap_y_pt * noise_pt)

            C = z_leak_pt**2 * self.R_lisn**3 * z_cap_y_pt + noise_pt * z_leak_pt**2 * self.R_lisn**3 + noise_pt * z_cap_y_pt * self.R_lisn**3 * z_leak_pt
...
class Noise_Limit_Avg:
    def __init__(self, freq_range: np.ndarray) -> None:
        self.freq = freq_range
        low_range = 66 # <- 60dBuV from FCC
        high_range = 60 # <- 66dBuV from FCC

        self.upper_mask = freq_range >= 0.5 * 10**6
        self.FCC = self.calc_limit(high_range, low_range)
